...
def formatRow(row, fieldnames):
	basic_dict = {
		...
		'Voter Street Address': titleCase(str(row['RESIDENTIAL_ADDRESS1']).strip()),
		...
		'Zip Code': str(row['RESIDENTIAL_ZIP']).strip(),
		'County': titleCase(county_map[str(row['COUNTY_NUMBER']).strip()]),
		'Precinct': str(row['PRECINCT_NAME']).strip(),
		'Precinct Split': str(row['PRECINCT_CODE']).strip(),
